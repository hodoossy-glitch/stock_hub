...
# 3. 상단 헤더: 실시간 시장 전광판 (에러 원천 차단 디자인)
...
for s_name, s_news in sectors.items():
    with st.expander(f"📂 {s_name} | {s_news}", expanded=True):
        ...
        if not live_df.empty:
            # 실시간 4% 이상 필터링 + 거래대금 순
            s_df = live_df[(live_df['Sector'].str.contains(s_name, na=False)) & (live_df['ChangesRatio'] >= 4.0)].sort_values('Amount', ascending=False).
