...
# 1. 페이지 설정 및 전체 화면 테마 전환 로직 (선생님의 다크 프레임 보존)
...
st.markdown(f"""
    ...
    /* 종목 카드 디자인 보존 */
    .stock-card {{ background-color: {card_bg}; padding: 10px; border-radius: 8px; border: 1px solid {border_color}; margin-bottom: 5px; text-align: center; min-height: 95px; }}
...
# 2. 데이터 엔진 (KeyError 및 TypeError 완벽 방지)
...
with tab1:
    ...
    with c1:
        st.markdown(f'<div class="m-header"><b>KOSPI</b><br><span class="big-num">2,642.15</span><br><small style="color:#ff4b4b;">▲ 1.38% ({t1.get("대금","16.3조")})</small></div>', unsafe_allow_html=True)
        if "hist" in t1: st
